...
**ADR-008 · 安全：防恶意/被诱导的上游 + 暴露面收敛**
- 背景：本地网关的主风险不是被打，而是连上**恶意 MCP server**，或 agent 诱导连上恶意上游；以及无意中把无鉴权服务暴露到局域网。
- 威胁模型澄清：本项目 config 由 host **静态加载**（TOML/env），上游/agent **无运行时改 config 的 API** —— "agent 动态改配置执行任意程序"在当前代码不存在。下列措施为纵深防御 + 为未来动态配置留闸。
- 决策：
  1. **stdio 命令白名单（opt-in）**：`GatewayConfig.allowed_commands`。空=放行（config 可信）；非空=stdio `command` 必须按全串或文件名匹配，否则 `validate()` 失败、`Gateway::build` 拒绝 spawn。**不硬编码 `airp-mcp`**（硬编码违反「通用、不捆绑」），由 operator 选择收紧。
  2. **默认 loopback**：`bind` 默认 `127.0.0.1:8080`（本就如此）。改 `0.0.0.0` 即 operator 显式 opt-in（无 CLI，故由 config 显式设定承担「--expose-lan」语义）。
  3. **暴露告警**：`run()` 检测到非 loopback 绑定且 `access_key` 为 None → 响亮 `warn`（不硬拒，保留外部鉴权/防火墙场景的自由）。
- 后果：通用性不受损（默认放行 + 默认 loopback），但给出 opt-in 收紧手段与危险组合告警。验证：`config.rs` 单测（白名单放行/拒绝/basename/HTTP 豁免）。
- 未做（留待 Stage 6）：上游响应大小/超时限制、SSRF（HTTP upstream url 指向内网）防护、命令 args 校验。

**ADR-010 · 通用性回拉：agentbus feature 化 + SSRF 放行 loopback**
- 背景：通用性复审（针对"吸引第三方把本项目当底层"）发现两处摩擦：(1) `agentbus`(State-Protocol 专属代码)无条件编进核心 crate，违 ADR-007 字面「外部/可选」，纯桥使用者也得编；(2) `block_private_upstream_urls=true` 默认拦 loopback，而本地网关最常见的 http 上游正是 `127.0.0.1` 上的 MCP server（e2e_http 当时被迫关闭该防御）。
- 决策：
  1. **`agentbus` 改 cargo feature，默认 OFF**：`#[cfg(feature="agentbus")] pub mod agentbus`；`uuid` 设 optional 绑该 feature；example/test 用 `required-features`。纯桥使用者零编译该项目专属代码 → 完全恢复「不付不用」。CI 跑默认(纯桥 build+test) + `--features agentbus` 两轮。
  2. **SSRF 放行 loopback**：`url_blocked_target_reason` 不再拦 `127.0.0.0/8`/`::1`/`localhost`；仍默认拦私网/link-local(含云元数据 169.254.169.254)/unspecified/保留段。本地 MCP 上游开箱即用。
- 后果：纯桥消费者编译面更小、无项目耦合；本地 http 上游无需翻 flag。验证：默认 31 测试 + `--features agentbus` 45 测试 + clippy all-features 全绿（本地 + CI）。
...
**ADR-009 · 健壮性 + 安全硬化（Stage 6 前置一批）**
- 背景：Stage 6 留项清单（ADR-008 未做 + §8 已知风险）需分批落地，避免一次性大改。
- 决策（本批）：
  1. **HTTP upstream SSRF 防护**：`GatewayConfig.block_private_upstream_urls`（默认 true），`validate()` 时对 HTTP upstream URL 做 loopback / unspecified / link-local / RFC 1918/RFC 6890 reserved / localhost hostname 静态校验。**不做 DNS 解析**（racy + ToCTOU），由 operator 显式 opt-out 改 false。
  2. **请求体大小上限**：`max_request_bytes`（默认 1 MiB），双层 gate —— `tower-http RequestBodyLimitLayer`（前端）+ `handlers::dispatch` 内重检（自建前端兜底）。
  3. **错误信息脱敏**：`GatewayError::is_client_safe()` —— Transport/Upstream/Io 错误只回 canonical reason，完整 detail 落 server 日志，防泄漏上游路径/内部细节给前端。
  4. **优雅关机**：`shutdown_signal()`（Ctrl-C / SIGTERM）→ `axum::serve::with_graceful_shutdown` → `pool.shutdown_all()`。
  5. **stdio 规范关机序列**：`StdioTransport::shutdown` = close stdin → wait ≤5s → start_kill；`Drop` best-effort kill。补 §8 遗留项「stdio 未实现规范的关机序列」。
  6. **stdio reader EOF drain**：子进程 stdout EOF / I/O error 时，所有 pending oneshot 发 `-32000 upstream process exited`，调用方不再永久挂起。
  7. **server notification 通道**：`StdioTransport::subscribe_notifications()`（broadcast 64），为 Stage 2 流式铺路（progress notification 可订阅）。
  8. **协议版本协商校验**：`SUPPORTED_PROTOCOL_VERSIONS`（含 2025-06-18 + 2025-03-26），服务端 negotiate 不支持的版本则断开（补 §8 + R2）。HTTP `MCP-Protocol-Version` 头用协商值。
  9. **UpstreamPool 构建回滚**：`from_config` 任一上游 connect 失败 → 已建 client 全 shutdown，不泄漏 transport。
- 后果：生产健壮性显著提升；SSRF 防护默认开（operator 显式 opt-out 才可指向内网，对本地 mock e2e 需设 `block_private_upstream_urls=false`）。未做（仍留 Stage 6）：请求超时、上游重连/健康检查、上游响应大小上限、stdio args 校验、metrics/追踪。
- 验证：config.rs SSRF 单测覆盖（loopback/localhost/private/link-local/unspecified/public/disable）；本机 check/clippy/fmt 全绿，完整 test 验证靠 CI。

---

## 4. 当前架构快照（当前真相）

状态：**Stage 1+3 完成，Stage 6 安全/健壮性首批已落地（ADR-009 + P2a-P2d），CI 全绿**。
...
| 鉴权（常数时间）/ CORS / 限流 | ✅ |
| 声明式路由匹配 + 分发 | ✅ |
| stdio 传输（请求-响应 + 规范关机 + EOF drain） | ✅ |
| http 传输（session-id + 协议版本头 + SSE 解析） | ✅ |
| MCP initialize 惰性握手 + 版本协商校验 | ✅ |
| 安全：SSRF 防御 / 命令白名单 / args 校验 / 请求体上限 | ✅ |
| 健壮性：超时 / 响应上限 / 错误脱敏 / 优雅关机 / 构建回滚 | ✅ |
| 故障注入 e2e（上游错误/崩溃/超时） | ✅ |
...
**R10 · 进一步改进优化全景（2026-06-29）**
- 背景：用户问「项目还能如何进一步改进优化迭代」。对全量源码 + ROADMAP + DESIGN + 当前分支未提交改动做了系统审读。
- 发现（当前分支 `feat/robustness-and-security-hardening` 未提交改动）：已实现 ADR-009 全部 9 条（SSRF / max_request_bytes / 错误脱敏 / 优雅关机 / stdio 关机序列 / EOF drain / notification broadcast / 版本协商校验 / 构建回滚）——对应 Stage 6 安全续 + 健壮性首批。
- 影响：剩余改进分 7 个维度（见 ROADMAP §三展开）：
  1. **P0**：提交当前分支改动 → CI 验证 → 合 main（前提）。
  2. **P1 · Stage 2 流式**：`McpTransport::request_stream` + stdio 过滤 progress notification + HTTP `bytes_stream()` SSE 增量 + `axum::Sse` + 背压/断连 cancel 传播。已有基础：broadcast channel、`parse_sse`、`DispatchOutcome::Stream` 桩、`RouteTarget::Tool{stream}`。
  3. **P2 · Stage 6 深化**：请求超时（`invoke` 包 `tokio::time::timeout`）+ 客户端断开取消传播；上游健康检查（ping tick）+ 自动重连；上游响应大小上限（`max_response_bytes`）；stdio args 校验。
  4. **P2 · 故障注入 e2e**：mock 上游崩溃 / 超时 / 慢响应 / 巨大响应 / 无效 JSON，断言 Gateway 不挂起、返回规范错误。
  5. **P3 · 可观测性**：prometheus metrics（requests_total / duration / upstream_health）+ `/metrics` 端点（feature flag）+ request_id 中间件 + 可选 JSON 日志格式。
  6. **P3 · Stage 4 路由增强**：路径参数 `{id}` 提取注入 tool arguments；可选 request/response 变换层（声明式 jmespath 或 WASM 沙箱，**不进 bridge**）；多上游 fan-out。
  7. **P5 · DX/生态**：配置校验扩展（路由引用上游存在性、path 不重复、启动配置摘要）；`/status` + `/routes` admin 端点（独立 admin_key）；OpenAPI 从 routes 自动导出；热重载（SIGHUP → 重载 routes）；性能基线（criterion + CI 回归告警）；属性测试（proptest 覆盖 parse_sse / SSRF 分类）。
- 原则重申：所有新功能走可选 feature flag 或外部 crate，**不破坏纯桥/库优先/传输无关/不捆绑四铁律**。变换层、metrics、admin API 不进核心 bridge。
- 决策：本批（ADR-009）在当前 PR 落地；后续分批迭代，每个 stage CI 全绿才合 main。

**R5 · 工具链 / 构建环境**

**R11 · 源码审读发现的具体改进点（2026-06-29）**
- 背景：CI 全绿后，对全量源码做第二轮审读，找 ROADMAP 之外的代码级改进。
- 发现（10 项，按优先级排序）：
  1. **HTTP 响应先全量读再检查大小（OOM 风险）**：`HttpTransport::request` 中 `resp.text().await` 把整个 body 读进内存，**然后**才检查 `text.len() > max_response_bytes`。恶意/buggy 上游发 1GB 响应 → Gateway 先分配 1GB 再拒绝。修复：先检查 `Content-Length` 头，再用 `resp.chunk()` 流式读取并累加计数。
  2. **超时后 pending oneshot 泄漏**：`send_with_timeout` 超时时 future 被 drop，但 `StdioTransport::pending` HashMap 中该 id 的 oneshot sender 仍在。后续子进程回带该 id 响应时 send 到已关闭 oneshot（无害），但 pending 表条目永不移除 → 内存缓慢泄漏。修复：超时后主动从 pending 移除，或给 `McpTransport::request` 加 `CancellationToken`。
  3. **Io/Json 错误脱敏不完整**：`GatewayError::Io/Json` 的 `is_client_safe()` 返回 `true`，但 `into_response` 把完整 `Display` 信息返回客户端 — 路径、文件描述符、JSON 解析细节全部暴露。修复：`into_response` 中对非 client-safe 变体返回泛化消息，完整信息落 tracing。
  4. **路由匹配 O(n) 线性扫描**：`Bridge::match_route` 遍历 `Vec<RouteRule>`。路由数少时无影响，50+ 路由有可观测开销。建议：启动时预构建 `HashMap<(Method, Path), &RouteRule>` 或引入 `matchit` 路由树。
  5. **配置校验不校验路由引用上游存在性**：`validate()` 校验 SSRF/白名单/args，但不校验路由的 `upstream` 字段是否在 `upstreams` 中存在。运行时 `dispatch` 才返回 `UnknownUpstream`。修复：`validate()` 中加引用完整性检查。
  6. **CI 缺 audit / MSRV / lock 检查**：当前 CI 只做 fmt + clippy + test + e2e。缺 `cargo audit`（依赖漏洞扫描）、MSRV 检查、`Cargo.lock` 提交 + `--locked` 构建。
  7. **stdio reader 无限行长（OOM 风险）**：`BufReader::new(stdout).lines()` 会把一行完整读入内存。恶意上游发一行 10GB → OOM。修复：设行长度上限（如 1 MiB），超限断开。
  8. **HttpTransport 超时常量未使用**：`CONNECT_TIMEOUT = 5` 和 `REQUEST_TIMEOUT = 30` 定义了但未在 `reqwest::Client` 构建时设置。修复：在 `new()` 中设置 `.connect_timeout()` / `.timeout()`，或删除常量。
  9. **reqwest stream feature 未使用**：`Cargo.toml` 启用了 `reqwest/stream`，但当前 `request()` 用 `resp.text()` 全量读取。`stream` feature 增加编译时间和二进制大小。建议：放到 feature flag `streaming` 下。
  10. **缺少 Cargo.lock 提交**：库 crate 通常可不提交 lock，但作为可嵌入网关，确定性构建重要。建议提交 `Cargo.lock`，CI 加 `--locked`。
- 影响：其中 1/7/3 为安全/稳定性漏洞，优先修复；5/8 为代码质量改进，成本极低；其余为中期优化。写入 ROADMAP §三 P0-P2 展开项。
- 发现：
...
- **2026-06-14** ADR-008 安全：opt-in stdio 命令白名单（`allowed_commands` + `validate()`，build 时拒绝 spawn）、默认 loopback（确认）、非loopback+无auth 暴露告警。config.rs 单测覆盖（放行/拒绝/basename/HTTP 豁免）。
- **2026-06-29** R10 + ADR-009：健壮性 + 安全硬化首批（SSRF 防御 / 请求体上限 / 错误脱敏 / 优雅关机 / stdio 规范关机序列 + EOF drain / notification broadcast / 协议版本协商校验 / 构建回滚），9 条全部落地于分支 `feat/robustness-and-security-hardening`（待 commit + CI）。R10 给出后续 7 维度改进全景与 P0-P5 优先级。ROADMAP §三补 Stage 6 安全续首批完成项 + §四版本规划同步。
...
- **2026-06-30** PR #1（hardening）+ PR #2（agentbus）审计 + 修复 + 依次合并入 main。#1 修：路由错误标签、SSRF v4 段收窄、stdio 缓冲块读(perf)。#2 修：conn-id 经 `?conn=`/`airp-ready` 可用、`ConnGuard` 修 SSE 订阅泄漏。
- **2026-06-30** ADR-010 通用性回拉：`agentbus` 改默认关 feature（`uuid` optional、example/test `required-features`、CI 双轮）；SSRF 默认放行 loopback。本地默认 31 测试 + `--features agentbus` 45 测试 + clippy all-features 全绿。dlltool(`D:\msys64`) 装妥后本地全量测试可用。
